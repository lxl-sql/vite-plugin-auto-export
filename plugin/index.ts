...
function generateIndexFile(options: GenerateIndexFileOptions) {
  const {
    dir,
...
function render(options: GenerateIndexFileOptions) {
  const { dir, outFile = defaultOutFile } = options;
...
function autoI18nPlugin(options: AutoExportOptions) {
  const { files, outFile = defaultOutFile, ...resetOptions } = options;
